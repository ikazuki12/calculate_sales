...
def main
  return puts("支店定義ファイルが存在しません") if empty?("branch.lst")
  unless branches = load("branch.lst", "^\\d{3}$", Branch)
    return puts("支店定義ファイルのフォーマットが不正です")
  end
  return puts("商品定義ファイルが存在しません") if empty?("commodity.lst")
  ...
  sales_files.each do |file|
    ...
    return puts("合計金額が10桁を超えました") if exceed_ten?(branches, sales_table[:header][0])
    return puts("#{file}の商品コードが不正です") unless calculate!(sales_table[:header][1], sales_table, commodities)
    return puts("合計金額が10桁を超えました") if exceed_ten?(commodities, sales_table[:header][1])
  end
...
def exceed_ten?(hash, code)
  not hash[code].price.to_s.length < 10
end
